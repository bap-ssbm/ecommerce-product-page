...
function AddItems(props) {
    ...
    const [amountToAdd, changeAmountToAdd] = useState(0);
    
    function increaseAmount(){
        changeAmountToAdd(amountToAdd + 1);
    }
    function decreaseAmount(){
        
        changeAmountToAdd(amountToAdd===0?0:amountToAdd-1)
    }
    function catchChange(event){
        const newNum = Number(event.target.value);
        changeAmountToAdd(previousnum=>{
            return isNaN(newNum)?previousnum: newNum;
    })}
